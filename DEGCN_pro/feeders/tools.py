...
def valid_crop_resize(data_numpy,valid_frame_num,p_interval,window): # 定义一个函数，用于裁剪和调整数据的大小
    # input: C,T,V,M
    C, T, V, M = data_numpy.shape # 获取数据的形状，其中 C 是通道数，T 是时间步数，V 是关键点数，M 是额外的维度
    begin = 0 # 定义裁剪的起始和结束位置
    end = valid_frame_num
    valid_size = end - begin # 计算有效帧的数量

    #crop
    if len(p_interval) == 1: # 如果 p_interval 的长度为 1，则使用该值进行中心裁剪，计算偏移量 bias，并从数据中提取中心部分
        p = p_interval[0]
        bias = int((1-p) * valid_size/2)
        data = data_numpy[:, begin+bias:end-bias, :, :]# center_crop
        cropped_length = data.shape[1]
    else: # 如果 p_interval 的长度大于 1，随机选择裁剪比例 p，计算裁剪长度，并随机选择裁剪的起始位置
    ...
    # resize
    data = torch.tensor(data,dtype=torch.float) # 将裁剪后的数据转换为 PyTorch 张量，调整维度以适应插值操作，然后使用双线性插值调整大小，最后再调整回所需的形状并转换为 NumPy 数组
