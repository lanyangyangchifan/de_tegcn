...
file_workdir = './data/'
file = ['train_joint','test_joint','val_joint'] #原始数据N,C,T,V,M
file_suffix = '.npy'
...
def rid_0():
    zero_matrix = torch.zeros((1, 2, 17, 3), dtype=torch.float32).to(device)
    for i in range(3):
        ...
        data_rid0 = None
        for sample in tqdm(range(data.size(0))):
