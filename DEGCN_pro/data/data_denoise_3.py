        ...
        for i in range(joint_data[f].shape[0]):
            ...
            if len(valid_frame) == 0:
                if f == 0:
                    ...
                    index = np.argwhere(joint_data[f][i][:,0]>1000)
                    for t,v,c in index:
            ...
            for j in range(joint_validframe[f][i]):
                if j not in valid_frame:
                    ...
                    index = np.argwhere(np.abs(joint_data[f][i][j,0])>1000)
                    for v,c in index:
        ...
        for i in double_index[f]:
            ...
            if len(valid_frame) == 0:
                if f == 0:
                    ...
                    index = np.argwhere(joint_data[f][i][:,1]>1000)
                    for t,v,c in index:
            ...
            for j in range(joint_validframe[f][i]):
                if j not in valid_frame:
                    ...
                    closest_index = np.argmin(absolute_diff)
                    closest_frame = valid_frame[closest_index]
                    index = np.argwhere(np.abs(joint_data[f][i][j,1]-joint_data[f][i][j,1][1])>1000)
                    for v,c in index:
...
def rotate_data(xaxis=[5, 6]):
    for i in range(3):
        N, T, M, V, C = joint_data[i].shape
        s = np.transpose(joint_data[i], [0, 2, 1, 3, 4])  # N, T, M, V, C  to  N, M, T, V, C
        # N, C, T, V, M = joint_data[i].shape
        # s = np.transpose(joint_data[i], [0, 4, 2, 3, 1])  # N, C, T, V, M  to  N, M, T, V, C
        for i_s, skeleton in enumerate(tqdm(s)):  # x轴对齐
            if skeleton.sum() == 0:
                continue
        ...
        joint_data[i] = np.transpose(s, [0, 2, 1, 3, 4])
